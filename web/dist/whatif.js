...
(() => {
  ...
  <div class="whatif-controls">
    <label>Rentestigning efter studiet <output id="wi-rise-value"></output><input aria-label="Rentestigning efter studiet i procentpoint" id="wi-rise" type="range" min="0" max="5" step="0.25" value="2"><small>0 til 5 procentpoint oven i renten på din beregnede plan</small></label>
    <label>Leveomkostninger efter studiet <output id="wi-cost-value"></output><input aria-label="Leveomkostninger efter studiet i kroner" id="wi-cost" type="range" min="0" max="40000" step="250" value="12000"><small>Pr. måned, uden betaling af SU-lånet</small></label>
    <label>Forventet nettoløn <output id="wi-income-value"></output><input aria-label="Forventet nettoløn i kroner" id="wi-income" type="range" min="0" max="50000" step="250" value="22000"><small>Udbetalt efter skat pr. måned – din egen antagelse</small></label></div>
  ...
  function update() {
    ...
    el('table').innerHTML=scenarios.map((s,i)=>{const reserve=Math.ceil(s.payment.maxPayment/s.interval),left=income-costs-reserve;return `<tr><th>${names[i]}</th><td>${percent(s.rate)}</td><td>${money(reserve)}</td><td class="${left<0?'negative':''}">${money(left)}</td><td>${s.payment.end || 'Ingen gæld'}</td></tr>`;}).join('');
    const s=scenarios[Number(el('selected').value)], reserve=Math.ceil(s.payment.maxPayment/s.interval),left=income-costs-reserve;
